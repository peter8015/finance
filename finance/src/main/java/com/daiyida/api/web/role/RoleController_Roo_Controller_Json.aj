...
import com.daiyida.api.web.role.RoleController;
...
privileged aspect RoleController_Roo_Controller_Json {
    ...
    @ResponseBody
    public ResponseEntity<String> RoleController.showJson(@PathVariable("id") String id) {
    ...
    public ResponseEntity<String> RoleController.createFromJson(@RequestBody String json, UriComponentsBuilder uriBuilder) {
        Role role = Role.fromJsonToRole(json);
        roleService.saveRole(role);
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json");
        RequestMapping a = (RequestMapping) getClass().getAnnotation(RequestMapping.class);
        headers.add("Location",uriBuilder.path(a.value()[0]+"/"+role.getId().toString()).build().toUriString());
    ...
    public ResponseEntity<String> RoleController.updateFromJson(@RequestBody String json, @PathVariable("id") String id) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json");
        Role role = Role.fromJsonToRole(json);
        role.setId(id);
    ...
    public ResponseEntity<String> RoleController.deleteFromJson(@PathVariable("id") String id) {
        Role role = roleService.findRole(id);
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json");
        if (role == null) {
